...
function Sidebar({ match, history }) {
    ...
    const sidebarStyle = {
        width: 280,
        left: window.innerWidth < 640 && !ToggleMenuSidebar ? "-280px" : 0
    }
    return (
        <>
            <div className="flex sm:hidden">
                <button onClick={() => setToggleMenuSidebar(prev => !prev)} className={[
                    "toggleSidebar z-50", ToggleMenuSidebar ? " active" : ""
                    ].join("")}>
                </button>
            </div>
            <aside className="transition-all duration-300 bg-indigo-1000 max-h-screen h-screen overflow-y-auto min-h-full fixed sm:relative z-50" style={sidebarStyle}>
                {ToggleMenuSidebar && <div className="overlay" onClick={() => setToggleMenuSidebar( prev => !prev )}></div>}
